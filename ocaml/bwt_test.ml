...
let expect_error e = function
  | Error e' when e = e' -> ()
  | Error e' -> Alcotest.failf "got error '%s', expected '%s'" e' e
  | Ok _ -> Alcotest.failf "got ok, expected error '%s'" e
;;

let expect_ok = function
  | Ok _ -> ()
  | Error e -> Alcotest.failf "got error: %s" e
;;
...
let test_int_of_safehex_known_values () =
  Alcotest.(check (result int string)) "G" (Ok 0) (Bwt.int_of_safehex "G");
  Alcotest.(check (result int string)) "H" (Ok 1) (Bwt.int_of_safehex "H");
  Alcotest.(check (result int string)) "Z" (Ok 15) (Bwt.int_of_safehex "Z");
  Alcotest.(check (result int string)) "HG" (Ok 16) (Bwt.int_of_safehex "HG");
  Alcotest.(check (result int string)) "ZZ" (Ok 255) (Bwt.int_of_safehex "ZZ");
  Alcotest.(check (result int string)) "HGG" (Ok 256) (Bwt.int_of_safehex "HGG")
;;

let test_int_of_safehex_empty () =
  Alcotest.(check (result int string))
    "empty" (Error "Bwt: bad safe-hex") (Bwt.int_of_safehex "")
;;

let test_int_of_safehex_invalid () =
  Alcotest.(check (result int string))
    "lowercase" (Error "Bwt: bad safe-hex") (Bwt.int_of_safehex "g");
  Alcotest.(check (result int string))
    "digit" (Error "Bwt: bad safe-hex") (Bwt.int_of_safehex "0");
  Alcotest.(check (result int string))
    "hex letter" (Error "Bwt: bad safe-hex") (Bwt.int_of_safehex "A");
  Alcotest.(check (result int string))
    "mixed valid/invalid" (Error "Bwt: bad safe-hex") (Bwt.int_of_safehex "GA");
  Alcotest.(check (result int string))
    "leading zero" (Error "Bwt: bad safe-hex") (Bwt.int_of_safehex "GG");
  Alcotest.(check (result int string))
...
let test_make_negative_issued_at () =
  expect_error "Bwt: integer overflow" @@ Bwt.make ~issued_at:(-1) ~user:0 30
;;

let test_make_zero_expires () =
  expect_error "Bwt: bad expiry: 0" @@ Bwt.make ~user:0 0
;;

let test_make_negative_expires () =
  expect_error "Bwt: bad expiry: -5" @@ Bwt.make ~user:0 (-5)
;;
...
let test_decode_wrong_key () =
  ...
  expect_error "Bwt: bad signature" @@ Bwt.decode ~today:bad_key s
;;
...
let test_decode_both_keys_invalid () =
  ...
  expect_error "Bwt: bad signature" @@ Bwt.decode ~yesterday:bad2 ~today:bad1 s
;;
